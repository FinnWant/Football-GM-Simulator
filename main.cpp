...
// NFL Teams Database
vector<NFLTeamInfo> nflTeams = {
    {"Bills", "Buffalo", "Sean McDermott"},
...
vector<string> firstNames = {
    ...
    "Daniel", "Joseph", "Steven", "Brian", "Kevin", "Jason", "Justin", "Ryan",
    "Eric", "Brandon", "Alexander", "Zachary", "Nathan", "Austin", "Hunter", "Dylan",
    "Blake", "Logan", "Evan", "Sean", "Tyler", "Landon", "Parker", "Matthew",
    "Andrew", "Mark", "Paul", "Kenneth", "George", "Scott", "Charles", "Anthony"
};
vector<string> lastNames = {
    "Smith","Johnson","Williams","Brown","Jones","Garcia","Miller","Davis","Rodriguez","Martinez",
    "Hernandez","Lopez","Gonzalez","Wilson","Anderson","Clark","Lee","Walker","Hall","Allen",
    ...
    "Cooper","Richardson","Cox","Howard","Ward","Torres","Peterson","Gray","Ramirez","James","Brooks"
};
vector<string> teamNames = {
    "Eagles","Lions","Bears","Vikings","Falcons","Panthers","Ravens","Bengals","Browns","Steelers",
    "Texans","Colts","Jaguars","Titans","Broncos","Sharks","Tigers","Armada","Sentinels","Cyclones",
    "Warriors","Avalanche","Comets","Rockets","Fusion","Guardians","Marauders","Dragons","Phoenix","Hornets",
    "Spartans","Knights","Outlaws","Renegades","Stallions","Riders","Legends","Pioneers","Mustangs","Trailblazers",
    "Thunder","Blizzard","Hurricanes","Miners","Raptors","Beacons","Savages","Phantoms","Barracudas","Outriders"
};
...
vector<string> coachFirstNames = {
    "Andy","Bill","Mike","John","Pete","Sean","Kyle","Matt","Bruce","Dan",
    "Jim","Ron","Frank","Todd","Doug","Rick","Eli","Greg","Nick","Ryan",
    "Hunter","Shane","Corey","Sam","Alex","Chris","Pat","Ed","Tom","Gary",
    "Mark","Steve","Vince","Leo","Walter","Martin","Oscar","Theo","Drew","Jordan"
};
...
player generateRandPlayer(string position) {
    ...
    if (position == "QB") {
        return quarterback(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "RB") {
        return runningback(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "WR") {
        return wide_receiver(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "TE") {
        return tight_end(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "OL") {
        return lineman(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "DL") {
        return defensive_lineman(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "LB") {
        return linebacker(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "DB") {
        return defensive_back(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99), randInt(50, 99));
    }
    else if (position == "K") {
        return kicker(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99));
    }
    else if (position == "P") {
        return punter(position, overall, number, age, name,
            randInt(50, 99), randInt(50, 99));
    }
    // Default case - should never reach here if position is valid
    return player(position, overall, number, age, name);
}
...
player createPlayer() {
    ...
    if (position == "QB") {
        ...
        return quarterback(position, overall, number, age, name, tp, ta, spd, pv);
    }
    else if (position == "RB") {
        int spd = getIntInput(50, 99, "Enter speed (50-99): ");
        ...
        return runningback(position, overall, number, age, name, spd, car, elu, blk);
    }
    else if (position == "WR") {
        int spd = getIntInput(50, 99, "Enter speed (50-99): ");
        int ctch = getIntInput(50, 99, "Enter catching (50-99): ");
        int rr = getIntInput(50, 99, "Enter route running (50-99): ");
        return wide_receiver(position, overall, number, age, name, spd, ctch, rr);
    }
    else if (position == "TE") {
        int spd = getIntInput(50, 99, "Enter speed (50-99): ");
        int ctch = getIntInput(50, 99, "Enter catching (50-99): ");
        int blk = getIntInput(50, 99, "Enter blocking (50-99): ");
        return tight_end(position, overall, number, age, name, spd, ctch, blk);
    }
        ...
        if (position == "OL") {
            return lineman(position, overall, number, age, name, str, blk, tkl);
        } else {
            return defensive_lineman(position, overall, number, age, name, str, tkl, blk);
        }
    }
    else if (position == "LB") {
        int spd = getIntInput(50, 99, "Enter speed (50-99): ");
        ...
        return linebacker(position, overall, number, age, name, spd, tkl, cvr);
    }
    else if (position == "DB") {
        int spd = getIntInput(50, 99, "Enter speed (50-99): ");
        int cvr = getIntInput(50, 99, "Enter coverage (50-99): ");
        int tkl = getIntInput(50, 99, "Enter tackling (50-99): ");
        return defensive_back(position, overall, number, age, name, spd, cvr, tkl);
    }
    else if (position == "K") {
        int kp = getIntInput(50, 99, "Enter kick power (50-99): ");
        int ka = getIntInput(50, 99, "Enter kick accuracy (50-99): ");
        return kicker(position, overall, number, age, name, kp, ka);
    }
    else if (position == "P") {
        int pp = getIntInput(50, 99, "Enter punt power (50-99): ");
        int pa = getIntInput(50, 99, "Enter punt accuracy (50-99): ");
        return punter(position, overall, number, age, name, pp, pa);
    }
...
team generateRandTeam() {
    ...
    for (int i = 0; i < 3; i++) roster.push_back(generateRandPlayer("QB"));
    for (int i = 0; i < 4; i++) roster.push_back(generateRandPlayer("RB"));
    for (int i = 0; i < 6; i++) roster.push_back(generateRandPlayer("WR"));
    for (int i = 0; i < 3; i++) roster.push_back(generateRandPlayer("TE"));
    for (int i = 0; i < 9; i++) roster.push_back(generateRandPlayer("OL"));
    for (int i = 0; i < 8; i++) roster.push_back(generateRandPlayer("DL"));
    for (int i = 0; i < 7; i++) roster.push_back(generateRandPlayer("LB"));
    for (int i = 0; i < 10; i++) roster.push_back(generateRandPlayer("DB"));
...
void displayTeam(const team& t) {
    ...
    for (const auto& p : t.get_players()) {
        positionGroups[p.get_position()].push_back(&p);
    }
    
    // Display each position group
    vector<string> positionOrder = {"QB", "RB", "WR", "TE", "OL", "DL", "LB", "DB", "K", "P"};
